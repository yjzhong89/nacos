...
public class ServiceManager implements RecordListener<Service> {
    ...
    public void registerInstance(String namespaceId, String serviceName, Instance instance) throws NacosException {
        // 创建服务，如果没有服务则创建一个空服务，然后注册实例，检查健康
        // 服务端在接收到请求后，需要检查该服务的实例可以挂载到哪个服务上
        // 这里只是创建一个service
    ...
    public void addInstance(String namespaceId, String serviceName, boolean ephemeral, Instance... ips) throws NacosException {
        ...
        synchronized (service) {
            // 获取service中的所有实例---clusterMap的初始化
            List<Instance> instanceList = addIpAddresses(service, ephemeral, ips);

            Instances instances = new Instances();
            instances.setInstanceList(instanceList);

            // consistencyService怎么实例化？(spring自动注入)
            // consistencyService --- DelegateConsistencyServiceImpl代理对象
    ...
    public void putService(Service service) {
        if (!serviceMap.containsKey(service.getNamespaceId())) {
            synchronized (putServiceLock) {
                if (!serviceMap.containsKey(service.getNamespaceId())) {
                    // 先往serviceMap当中put一个key，但是value是一个空map
